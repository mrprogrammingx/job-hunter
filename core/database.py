...
def save_job(job: Job) -> int:
    ...
    try:
        cur = conn.execute(
            """INSERT OR IGNORE INTO jobs (title, company, location, url, source, date_posted, description)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (job.title, job.company, job.location, job.url, job.source, job.date_posted, job.description),
        )
        conn.commit()
...
def save_profile(profile: CandidateProfile) -> int:
    conn = get_connection()
    try:
        cur = conn.execute(
            """INSERT INTO profiles (name, email, phone, skills, experience, education, projects, certifications, raw_text)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                profile.name, profile.email, profile.phone,
                json.dumps(profile.skills), json.dumps(profile.experience),
                json.dumps(profile.education), json.dumps(profile.projects),
                json.dumps(profile.certifications), profile.raw_text,
            ),
        )
        conn.commit()
...
def get_latest_profile() -> Optional[CandidateProfile]:
    conn = get_connection()
    try:
...
def save_match(match: MatchResult) -> int:
    ...
    try:
        cur = conn.execute(
            """INSERT OR REPLACE INTO match_results (job_id, profile_id, score, skill_gaps, matching_skills, recommendation, reasoning)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (
...
def upsert_application(app: Application) -> int:
    conn = get_connection()
    try:
        cur = conn.execute(
            """INSERT INTO applications (job_id, status, date_discovered, date_applied, match_score, notes)
               VALUES (?, ?, ?, ?, ?, ?)
               ON CONFLICT(job_id) DO UPDATE SET
                 status=excluded.status, date_applied=excluded.date_applied,
                 match_score=excluded.match_score, notes=excluded.notes""",
            (app.job_id, app.status, app.date_discovered, app.date_applied, app.match_score, app.notes),
        )
        conn.commit()
        return cur.lastrowid or conn.execute("SELECT id FROM applications WHERE job_id=?", (app.job_id,)).fetchone()["id"]
    finally:
        conn.close()
    ...
    try:
        cur = conn.execute(
            """INSERT INTO application_materials
               (job_id, tailored_resume, cover_letter, recruiter_message, linkedin_message, interview_questions, interview_tips)
               VALUES (?, ?, ?, ?, ?, ?, ?)
               ON CONFLICT(job_id) DO UPDATE SET
                 tailored_resume=excluded.tailored_resume, cover_letter=excluded.cover_letter,
                 recruiter_message=excluded.recruiter_message, linkedin_message=excluded.linkedin_message,
                 interview_questions=excluded.interview_questions, interview_tips=excluded.interview_tips""",
            (
...
def get_materials(job_id: int) -> Optional[ApplicationMaterials]:
    conn = get_connection()
    try:
        row = conn.execute("SELECT * FROM application_materials WHERE job_id = ?", (job_id,)).fetchone()
        if not row:
            return None
        d = dict(row)
